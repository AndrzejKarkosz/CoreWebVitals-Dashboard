...
from google.analytics.data_v1beta.types import (
  ...
  script_path = '~/projects/looker-studio-pyscripts/Core Web Vitals/Core_Web_Vitals_CLS_ETL.py'
  ...
  #Set-up logs
  logging.basicConfig(filename = 'CLS_ETL_logs.log', level = logging.INFO, format = '%(asctime)s - %(message)s', datefmt = '%d-%b-%y %H:%M:%S1')
  ...
  request = RunReportRequest(
        ...
        dimensions=[
          ...
          Dimension(name="customEvent:largest_shift_target"),
          Dimension(name="customEvent:page_location_clean"),
          ],
        metrics=[
          Metric(name="customEvent:metric_value"),
          Metric(name="customEvent:largest_shift_time"),
...
df_raw['customEvent:largest_shift_time'] = df_raw['customEvent:largest_shift_time'].astype('float')
...
df_combined.rename(columns = {'customEvent:largest_shift_time': 'largest_shift_time'}, inplace = True)
df_combined.rename(columns = {'customEvent:largest_shift_target': 'largest_shift_target'}, inplace = True)
...
schema=[
        ...
        bigquery.SchemaField("largest_shift_target","STRING", mode="REQUIRED"),
        bigquery.SchemaField("page_location_clean","STRING", mode="REQUIRED"),
        bigquery.SchemaField("largest_shift_time","INTEGER", mode="REQUIRED"),
